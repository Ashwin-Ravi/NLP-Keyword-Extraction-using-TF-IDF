...
print 'Obtaining top 10 words from TF IDF....\n'
#getting top 10 comments from TF IDF algorithm (code written in Mytfidf.py)
topwords=[]
for i in range(0,10):
    #TF IDF using sklearn / library
    tfidf = TfidfVectorizer(analyzer='word', ngram_range=(1,3), min_df = 0, stop_words = 'english')
    tfs = tfidf.fit_transform(completeComment.split(" "))
    joinedComment=''
    response = tfidf.transform([joinedComment.join(preprocessing(((Thread_result[i])[1])))])
    feature_names = tfidf.get_feature_names()
    feature_array = np.array(tfidf.get_feature_names())
    tfidf_sorting = np.argsort(response.toarray()).flatten()[::-1]
    top10 = feature_array[tfidf_sorting][:10]
    topwords.append(top10)
